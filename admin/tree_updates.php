	    ...
	    {  
	         ...
	         $da = move_uploaded_file($file_tmp,"../uploads/tree_updates/".$file_name);
	            ...
	            {  
	               ...
	               $added_at = date('Y-m-d');
				   $sql="INSERT INTO  tree_updates (plant_id,pictures,added_at,garden_id) VALUES(:tree_id,:pictures,:added_at,:garden_id)";
...
$sql ="SELECT plant_id,tree_code,tree_status,planted_trees.added_at,tree_category_name,user_fname,user_lname,plant_tree_status,number_of_trees,location.location_id,location_name,tree_planted_at,tree_qr_code,category_image FROM planted_trees  LEFT JOIN tree_category ON  planted_trees.tree_category_id = tree_category.tree_category_id LEFT JOIN garden ON  planted_trees.garden_id = garden.garden_id LEFT JOIN location ON  garden.location_id = location.location_id  LEFT JOIN users ON  planted_trees.user_id = users.user_id 
    WHERE NOT EXISTS (SELECT * FROM tree_updates  WHERE planted_trees.plant_id = tree_updates.plant_id) AND planted_trees.garden_id = :garden_id  AND MONTH( tree_update_date) = :tree_update_date  ORDER BY planted_trees.plant_id desc";
$query=$dbh->prepare($sql);
$query->bindParam(':garden_id',$garden_id,PDO::PARAM_STR);
$query->bindParam(':tree_update_date',$date,PDO::PARAM_STR);
$query->execute();
	  		...
	  		<form method="POST" action="" id="add_garden" enctype="multipart/form-data">
	  		    <div class="form-group">
					<label class="control-label mb-10" for="exampleInputEmail_1">Tree Images</label>
						...
						<input class="form-control"  type="hidden" exampleInputuname_1" placeholder="" name="garden_id" value="<?php echo $result->garden_id;?>">
